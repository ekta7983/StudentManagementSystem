...
-- 2. UPDATE operations
-- Update a student's information by StudentID
UPDATE Students SET Email = 'newemail@example.com', PhoneNumber = '9123456789' WHERE StudentID = 4;

-- Update a course's information by CourseID
UPDATE Courses SET CourseDuration = 12, CourseFee = 21000.00 WHERE CourseID = 2;

-- Update an enrollment's date by EnrollmentID
UPDATE Enrollments SET EnrollmentDate = '2024-03-10' WHERE EnrollmentID = 1;
...
-- Find all students enrolled in 'Web Development'
SELECT 
    s.FirstName, s.LastName, c.CourseName, e.EnrollmentDate
FROM 
    Students s
...
-- Find courses taken by 'Priya Verma'
SELECT 
    s.FirstName, s.LastName, c.CourseName, e.EnrollmentDate
FROM 
    Students s
